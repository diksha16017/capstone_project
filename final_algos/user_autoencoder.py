...
user_item_matrix = np.zeros((totalUsers, totalMovies))
binary_matrix = np.zeros((totalUsers, totalMovies))
...
with open(training_dataset) as myFile:
    for line in myFile:
        ...
        movie = int(line[1]) - 1
        rating = int(line[2])
        user_item_matrix[user, movie] = rating
        if rating != 0:
            binary_matrix[user, movie] = 1
...
with open(test_dataset) as my_file:
    for line in my_file:
        ...
        number_of_records += 1
        actual_predicted_rating = predicted_ratings[user][movie] * 1
        if actual_predicted_rating > 1:
            actual_predicted_rating = 1
        if actual_predicted_rating < -1:
            actual_predicted_rating = -1
        print actual_rating,(actual_predicted_rating)
